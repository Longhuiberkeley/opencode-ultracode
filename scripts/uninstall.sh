...
OURS_ENTRIES=(index.ts tui.tsx .ultracode-install src skills node_modules package.json LICENSE)
...
if [[ -L "$PLUGIN_DIR" ]]; then
  echo "leaving $PLUGIN_DIR (symlink — remove it manually)"
elif [[ -d "$PLUGIN_DIR" ]]; then
  # An empty dir (interrupted install) is always safe to remove.
