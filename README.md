...
| `maxLoopDepth` | number | `2` | Hard cap on `loop()` nesting inside one run (engine-owned preflight in the worker; deeper nesting fails before iteration 1). Budgets are shared across nested loops, so this bounds structural blowup only. |
