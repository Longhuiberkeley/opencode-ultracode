...
blocked by construction (pin-pool and explicit candidates are the user's own
choices). Those rungs activate only when model catalog metadata becomes available
to the plugin. When no candidate is eligible — or the host lacks `switchModel` — the
child fails with the typed quota error, so the run stops visibly instead of burning
tokens.

**Pin your agents.** Children without a resolved model (no per-call / run override
and no agent pin, so the server default is used) cannot be provider-routed by the
breaker today: they never fail over, and they also bypass provider admission
(fail-closed). Users on coding plans should pin their agents.
...
PAUSED through the normal pause machinery (the watchdog is suspended, so `timeoutMs`
is not burned) and ONE coalesced report is emitted to the parent: provider, class,
reset time when known, affected child count, the proposed fallback from the ladder,
and the exact resume invocation. Burst throttle never pauses — it serializes
admission only; burst children same-model-retry with backoff. Children already
inside a failover park at the pause gate, so the ask really gates the routing.
Answer with:
...
`agent()` result carries `failover: { from, to, class: "quota" | "burst", reason }`. Warm reruns never replay a
...
when you want a stable fallback list instead of pin-pool inference.
